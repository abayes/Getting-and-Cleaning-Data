...
* The zip file needs to be downloaded to the working directory and unzipped prior to running run_analysis.R
* The necessary files the working directory should contain are as follows:
    * run_analysis.R
	* getdata-projectfiles-UCI HAR Dataset - This should be the default set-up, but just to be sure it needs to look like this
	    * UCI HAR Dataset
		    * test
		        * subject_test.txt
			    * X_test.txt
...
2. Load variable names and activity labels
...
7. Create a tidy dada set with the average of each variable for each activity and each subject
